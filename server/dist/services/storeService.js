...
export async function searchStores(request) {
...
function findShortestStore(stores) {
    return stores.reduce((closest, current) => current.distance < closest.distance ? current : closest);
}
function findHealthiestStore(stores) {
...
function findBudgetFriendlyStore(stores) {
...
function getHealthBonus(store) {
    ...
    let bonus = 0;
    if (storeCategories.some(cat => cat.includes('organic')))
        bonus += 2;
    if (storeCategories.some(cat => cat.includes('natural')))
        bonus += 1.5;
    if (storeCategories.some(cat => cat.includes('fresh')))
        bonus += 1;
    if (storeCategories.some(cat => cat.includes('premium')))
        bonus += 0.5;
    return Math.min(bonus, 3); // Cap at 3 bonus points
}
// Bonus points for budget-friendly stores
function getPriceBonus(store) {
    const budgetCategories = ['Discount', 'Budget-Friendly', 'Bulk Shopping', 'Warehouse'];
    const storeCategories = store.categories.map(cat => cat.toLowerCase());
    let bonus = 0;
    if (storeCategories.some(cat => cat.includes('discount')))
        bonus += 2;
    if (storeCategories.some(cat => cat.includes('budget')))
        bonus += 1.5;
    if (storeCategories.some(cat => cat.includes('bulk')))
        bonus += 1;
    if (storeCategories.some(cat => cat.includes('warehouse')))
        bonus += 1;
    return Math.min(bonus, 3); // Cap at 3 bonus points
}
...
function getMockStores(userLocation) {
    const mockStores = [
        {
            id: 'mock-1',
            name: 'Fresh Market',
            ...
            distance: calculateDistance(userLocation.latitude, userLocation.longitude, userLocation.latitude + 0.001, userLocation.longitude + 0.001),
            rating: 4.5,
            priceScore: 7,
            healthScore: 8,
        ...
        },
        {
            ...
            distance: calculateDistance(userLocation.latitude, userLocation.longitude, userLocation.latitude - 0.002, userLocation.longitude + 0.002),
            rating: 4.0,
            priceScore: 9,
            healthScore: 6,
        ...
        },
        {
            ...
            longitude: userLocation.longitude - 0.001,
            distance: calculateDistance(userLocation.latitude, userLocation.longitude, userLocation.latitude + 0.003, userLocation.longitude - 0.001),
            rating: 4.8,
            priceScore: 5,
            healthScore: 9,
