...
async function seedSampleData(): Promise<void> {
  ...
  const existingStores = db.prepare('SELECT COUNT(*) as count FROM stores').get() as { count: number }
  ...
  for (const store of sampleStores) {
    insertStore.run(
      ...
      store.latitude, store.longitude, store.price_score, store.health_score,
      store.rating, store.categories, store.hours
    )
  }
