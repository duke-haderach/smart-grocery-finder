...
async function searchZipcodesWithGoogle(query: string): Promise<ZipcodeResult[]> {
  ...
  try {
    ...
    if (response.data.status === 'OK') {
      const suggestions: ZipcodeResult[] = []
...
function extractZipcodeFromPrediction(prediction: any): string | null {
...
async function getLocationDetails(placeId: string, apiKey: string) {
  try {
...
const POPULAR_ZIPCODES: ZipcodeResult[] = [
...
export async function searchZipcodes(query: string): Promise<ZipcodeResult[]> {
  ...
  const fallbackResults = POPULAR_ZIPCODES
    .filter(location => {
      return (
        location.zipcode.startsWith(query) ||
