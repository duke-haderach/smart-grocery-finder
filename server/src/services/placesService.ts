...
import type { GroceryStore, GeocodeResult } from '../types/index'
...
export class PlacesService {
  ...
  async searchGroceryStores(userLocation: GeocodeResult, radiusMeters: number = 25000): Promise<GroceryStore[]> {
    if (!this.apiKey) {
    ...
    try {
      ...
      const storeTypes = [
        ...
        'food',
        'store',
        'department_store',        // Target, Walmart, Meijer
        'general_contractor',      // Sometimes used for big box stores
        'establishment',           // Catch-all category
  ...
  private convertPlacesToGroceryStores(places: any[], userLocation: GeocodeResult): GroceryStore[] {
  ...
  private analyzeStoreType(name: string, types: string[]): {
    priceScore: number
    healthScore: number
    categories: string[]
  } {
  ...
  private convertOpeningHours(openingHours: any): any {
    ...
    const dayMap: Record<string, keyof typeof defaultHours> = {
      'Monday': 'monday',
      'Tuesday': 'tuesday', 
      'Wednesday': 'wednesday',
      'Thursday': 'thursday',
